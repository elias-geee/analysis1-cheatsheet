...
\begin{itemize}
    \item \key{Pythagoras}: $\sin^2x + \cos^2x = 1$, $\;1+\tan^2x=\sec^2x$, $\;1+\cot^2x=\csc^2x$
    \item \key{Parität}: $\sin(-x)=-\sin x$, $\;\cos(-x)=\cos x$
    \item \key{Summe}: $\sin(a\pm b)=\sin a\cos b\pm\cos a\sin b$, $\;\cos(a\pm b)=\cos a\cos b\mp\sin a\sin b$
    \item \key{Doppelwinkel}: $\sin(2x)=2\sin x\cos x$, $\;\cos(2x)=\cos^2x-\sin^2x=2\cos^2x-1=1-2\sin^2x$
    \item \key{Halbwinkel}: $\sin^2\frac{x}{2}=\frac{1-\cos x}{2}$, $\;\cos^2\frac{x}{2}=\frac{1+\cos x}{2}$, $\;\tan\frac{x}{2}=\frac{1-\cos x}{\sin x}=\frac{\sin x}{1+\cos x}$
