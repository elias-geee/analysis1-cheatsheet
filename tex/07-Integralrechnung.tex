...
\Def \key{LIATE-Regel}: Wahl von $f$ (ableiten) nach Priorität: \textbf{L}ogarithmus, \textbf{I}nverse trig. Funk.,
\textbf{A}lgebraische Funk. (Polynom), \textbf{T}rig. Funk., \textbf{E}xponentialfunk. (Rest wird $g'$)
...
\Satz \key{Kriterium für gleichmässige Konvergenz}: Sei $M_n := \sup\limits_{x\in D}|f_n(x)-f(x)|$.
$(f_n)_{n\in\NN_0}$ konvergiert gleichmässig gegen $f$ $\iff$ $\limsup\limits_{n\to\infty} M_n = 0$

\Kor $I$ beschränktes Intervall, $(f_n)$ diff.bar auf $I$ mit $\sup\limits_{x\in I}|f_n'(x)| \to 0$.
Konvergiert $(f_n(x_0))$ für ein $x_0 \in I$, so konvergiert $(f_n)$ gleichmässig auf $I$
(folgt aus MWS: $|f_n(x) - f_n(x_0)| \le \sup_{x\in I}|f_n'(x)| \cdot |x - x_0|$)
...
\Kor \key{Ableitung von Integralen mit variablen Grenzen}: $f$ stetig, $a, b: I \to \RR$ diff.bar. Dann
$$\frac{d}{dx}\int_{a(x)}^{b(x)} \hspace{-4pt} f(t)\,dt = f(b(x))\,b'(x) - f(a(x))\,a'(x)$$
