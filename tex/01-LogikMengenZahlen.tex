...
$$e^{it} = \cos(t) + i\sin(t)$$
(komplexe Form von $\cos$/$\sin$ $\to$ Abschnitt 10)
...
\Satz \key{Umrechnung kartesisch $\leftrightarrow$ polar}: Für $z = a + ib = r e^{i\varphi}$ gilt:
\vspace{-0.6em}
\begin{itemize}
    \item \key{kart. $\to$ polar}:
    $$r = \sqrt{a^2+b^2}, \quad \varphi = \begin{cases}
        \arctan(b/a) & a > 0 \\
        ...
        -\pi/2 & a = 0, b < 0
    \end{cases}$$
    \item \key{polar $\to$ kart.}: $a = r\cos(\varphi)$, $b = r\sin(\varphi)$
\end{itemize}
